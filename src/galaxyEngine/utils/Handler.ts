...
export class Handler {
    ...
    run(): any {
        ...
        this._id === id && this.once && this.recover();  //防止重复回收
        return result;
    }
    ...
    runWith(data: any): any {
        ...
        if (data == null)
            var result: any = this.method.apply(this.caller, this.args);
        else if (!this.args && !data.unshift) result = this.method.call(this.caller, data);  //!data.unshift非数组
        else if (this.args) result = this.method.apply(this.caller, this.args.concat(data)); //concat合并两个数组元素 若data为数组调用时需要[data]防止元素被打散
